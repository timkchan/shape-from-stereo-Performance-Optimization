...
void calcDepthOptimized(float *depth, float *left, float *right, int imageWidth, int imageHeight, int featureWidth, int featureHeight, int maximumDisplacement)
{
	...
	int x_i = (imageHeight + maximumDisplacement) * imageWidth + imageWidth - 2 * featureWidth + maximumDisplacement; //mX
	int N = imageHeight * imageWidth + imageWidth - featureHeight + featureWidth; //mY
	//DONT FORGET TO FREEEEEEEEEEREEEEEEEEEEREEEEEEEEEEREEEEEEEEEE!!!!!
	float **c = malloc( N * sizeof( float* ));
	/* N is the number of rows  */
	/* note: c is char** */
	if (( c = malloc( N*sizeof( float* ))) == NULL ) {};
	//{ /* error */ }
	...
	for ( int i = 0; i < N; i++ )
	{
	   ...
	   */
	  if (( c[i] = calloc( x_i, sizeof(float) )) == NULL )
	  { /* error */ }
	  	//for (int j = 0; i < x_i; j++) c[i][j] = -1;
				...
				for (int dx = -maximumDisplacement; dx <= maximumDisplacement; dx++)
				{
					...
					for (int boxY = -featureHeight; boxY <= featureHeight; boxY++)
					{
						...
						for (int boxX = 0; boxX < featureWidth  * 2; boxX += 2)	//f/2*4 = 2f/4*4
						{
							...
							//if(squaredDifferenceArray[][])
							if (c[leftRX][rightRX] == 0) c[leftRX][rightRX] = 1;
